...
$Repo = "lun-4/makima"
$Binary = "makima"
$InstallDir = if ($env:MAKIMA_INSTALL_DIR) {
    $env:MAKIMA_INSTALL_DIR
} else {
    Join-Path $env:LOCALAPPDATA "makima"
}
...
function Install-Makima([string]$Tag) {
    $target = Get-Target
    if (-not $Tag) {
        $Tag = Get-LatestTag
    }
    ...
    $tmp = Join-Path ([System.IO.Path]::GetTempPath()) ("makima-install-" + [guid]::NewGuid().ToString("N"))
    ...
    try {
        ...
        try {
            ...
            Write-Err "failed to install to $dest (try running as Administrator or set MAKIMA_INSTALL_DIR): $_"
        }
