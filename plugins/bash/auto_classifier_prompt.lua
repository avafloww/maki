-- Bundled system prompt for the bash auto-mode classifier (codex-guardian).
-- Ported verbatim from pi-luna's `src/parts/classifier-prompt.md`.
-- Must be a `.lua` module: bundled plugins can only `require` `.lua` files
-- (a sibling `.md` is not runtime-readable), and this is the default `system`
-- for the classifier session.
