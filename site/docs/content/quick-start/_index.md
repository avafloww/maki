...
- **Sessions.** `/new` starts a second session while the first keeps working in the background; `/sessions` jumps between them. Tomorrow, `maki -l` resumes where you left off, `maki sessions --json` lists stored sessions, and `maki -c` picks one.
